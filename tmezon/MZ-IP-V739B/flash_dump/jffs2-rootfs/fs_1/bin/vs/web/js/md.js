...
$("#apply").click(function(){
	...
	for(i=0; i<maxrow; i++)
	{
		for(j=0; j<maxcol; j++)
		{
			...
			document.getElementById('Form2MdRegion' + m ).value = colbit;
...
function SaveTimePlan()
{
	...
	for(i=weekstart;i<weekend;i++)/* work or week*/
	{
		if((weekend == 9)&&(i == 7))/*Use the work mode */
		{	
			obj = document.getElementById('Form2Wrkday');	
		}
		else if((weekend == 9)&&(i == 8))/*Use the work mode */
		{
			obj = document.getElementById('Form2Wkend');	
		}
		else/*Use the week mode */
		{
		...
		if($("#TimeFrame")[0].contentWindow.$("input:checkbox[name='daycheck']").get(i).checked == true)/*当天有效*/
		{
			...
			{					
				...
				/*时间合法性检查*/
				if(startTime > endTime)/*非法*/
				{
					alert(MdErrorMsg);
					return false;
				}
				else if(startTime == endTime)/*如果时间相等，则不处理*/
				{
					continue;
				}
				else/*开始时间小于结束时间*/
